  ...
  return (
    ...
    >
      <Link to={`/products/${product.handle}`} className="block relative overflow-hidden">
        ...
        >
          <FiHeart size={16} className={isFavorite ? "fill-[#c4b5fd] text-[#c4b5fd]" : "text-white"} />
        </button>
        ...
        <div className={`absolute bottom-0 inset-x-0 flex justify-center space-x-2 bg-gradient-to-t from-[#0f172a] py-4 transform transition-all duration-300 ${
        ...
        <div className="pt-4 pb-3 px-3">
          <h3 className="product-title truncate text-[#f8fafc] group-hover:text-[#c4b5fd] transition-colors">{product.title}</h3>
          <div className="flex justify-between items-center mt-2">
            <div className="flex items-center">
              <span className="product-price text-[#c4b5fd] font-medium">Rs. {product.price.toFixed(2)}</span>
              {product.compareAtPrice && (
                <span className="product-price line-through text-[#94a3b8] text-xs ml-2">Rs. {product.compareAtPrice.toFixed(2)}</span>
              )}
            </div>
            {product.compareAtPrice && (
