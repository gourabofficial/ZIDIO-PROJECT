...
const AdminSettingsTrendingItems = ({ selectedIds = [], onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("Error saving trending items:", error);
      setError("Failed to save trending items. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center">
            ...
            <h3 className="text-white font-medium">Trending Products</h3>
          </div>
          <div className="bg-green-900/40 text-green-300 text-xs font-medium px-2.5 py-1 rounded-full">
        ...
        <div className="min-h-[180px] bg-gray-800/50 rounded-lg border border-gray-700 p-2 overflow-hidden shadow-inner">
          {loading ? (
            <div className="flex justify-center items-center h-full py-12">
              <div className="flex flex-col items-center">
                <div className="relative">
                  ...
                  <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-green-400 absolute top-0 left-0" style={{animationDuration: '1.2s'}}></div>
                </div>
              ...
              {productDetails.map((product) => (
                <div
                  ...
                  className="bg-gray-750 rounded-lg overflow-hidden transition-all duration-300
                    border border-gray-650 flex items-center group hover:shadow-md hover:border-green-800/40"
                >
            ...
            <div className="w-full flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-gray-300 font-medium">No trending products selected</p>
              <p className="text-gray-500 text-sm mt-1 max-w-sm">
                Add products to showcase in the Trending section of your homepage
              </p>
              <button
                className="mt-4 px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-md transition-all duration-200 flex items-center gap-2 font-medium"
      ...
      {/* Help text */}
      <div className="bg-green-900/20 border border-green-700/30 rounded-lg px-4 py-3 text-green-200 text-sm flex items-start">
        <Info className="text-green-400 mr-2 flex-shrink-0 mt-0.5" size={16} />
        <div>
          <p className="font-medium mb-1">Trending products drive customer engagement</p>
          <p className="text-green-300 opacity-80">
            These products will be displayed in the Trending section of your homepage. Choose popular items that are getting significant attention.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 justify-between items-center">
        <button
          className="px-4 py-2.5 bg-green-600 hover:bg-green-500 text-white rounded-md transition-all duration-200 flex items-center gap-2 font-medium shadow-sm hover:shadow"
          onClick={() => setIsPopupOpen(true)}
        >
          <PackagePlus size={16} />
      ...
      <ProductSearchPopup
        ...
        title="Select Trending Products"
        accentColor="green"
      />
    </div>
  );
